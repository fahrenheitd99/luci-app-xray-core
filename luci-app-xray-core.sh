...
while true; do
    ...
    if [ -z "$GEOSITE_URL" ] && [ -z "$GEOIP_URL" ]; then
        echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
        echo "@                                                                              @"
        echo "@  Warning! Core will not be started without geofiles!                         @"
        ...
        case "$CONFIRM" in
            [yY][eE][sS]|[yY])
                ...
                ;;
            *)
                echo "===> Retrying geofiles URLs configuration..."
                echo ""
                ;;
        esac
    else
...
echo "===> Registering CLI wrapper in /etc/profile..."
sed -i '/# XRAY_CLI_START/,/# XRAY_CLI_END/d' /etc/profile

cat << 'EOF' >> /etc/profile
# XRAY_CLI_START
xray() {
    if [ "$1" = "update" ]; then
        echo "===> Updating Xray core and system packages..."
        apk update && apk upgrade xray-core kmod-nft-tproxy
        /etc/init.d/xray restart
        ...
        echo "===> Removing routing and nftables rules..."
        ...
        echo "===> Removing configs, geofiles and LuCI interface..."
        rm -rf /etc/xray
        ...
        apk del xray-core kmod-nft-tproxy 2>/dev/null || true

        echo "===> Cleaning CLI wrapper from /etc/profile..."
        sed -i '/# XRAY_CLI_START/,/# XRAY_CLI_END/d' /etc/profile
        unset -f xray 2>/dev/null || true

        echo "===> System completely cleaned from Xray setup!"
    else
        /usr/bin/xray "$@"
    fi
}
